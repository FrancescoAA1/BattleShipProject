...
int main()
{
    ...
    p.add_ships(b, 5);
    p.add_ships(a, 5);
    p.receive_attack(Position{0, 1});
    p.receive_attack(Position{0, 2});
    p.receive_attack(Position{0, 3});
    p.receive_attack(Position{2, 0});
    p.receive_attack(Position{2, 1});
    p.receive_attack(Position{2, 3});
    std::cout<<p.defense_map(); 
    std::vector<AttackUnit> att;
    p.get_ship(Position{1, 1})->action(Position{1, 2}, att);
